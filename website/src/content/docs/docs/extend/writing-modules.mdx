...
Removal must write nothing to stdout and exit successfully. It succeeds only
after the target has been unregistered from Git and its directory no longer
exists. If a provider violates the exit or stdout contract after the irreversible
deletion already completed, Luvus reconciles the proven filesystem and Git state
instead of leaving a stale workspace; this recovery does not make stdout a
supported result channel. Luvus validates that the removal target is a linked
worktree of the source repository before running the command and verifies the
result afterwards. If `remove_command` is omitted, explicit removal uses
built-in Git.
