...
| opencode | structured SQLite session counters, including V2 `session_v2` |
...
OpenCode V2.0.2 uses the official `opencode` executable. This personal build
discovers its root sessions and structured usage through bounded read-only
SQLite queries. The optional CLI integration reports exact live ownership;
it installs into `cli.json`, separately from the V1 TUI plugin. The compatibility
command `opencode2` retains its older Luvus identifier and exact-ID resume.
Scheduled work for either command requires explicit **Full access** through
`opencode run --auto` or `opencode2 run --auto`.
