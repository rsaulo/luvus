...
Luvus also detects Antigravity, Amp, Devin, and Letta Code, but they are not
available as new automation workers. Antigravity, Amp, and Letta Code do not
expose a reviewed unattended permission mapping for Luvus access levels, and
Devin's workspace-trust gate has not been validated for unattended runs. They
can still appear when an automation targets a live agent that is already
running in a pane.
