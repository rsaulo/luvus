...
| `luvus uhp access --control` | Bounded control | `read`, `workspace`, `agent`, `terminal` | One use, at most 5 minutes | 24 hours by default; `--ttl` accepts 1 second to 24 hours |
...
advertised read surface plus selected focus, prompt, and terminal-control
methods. Discover the exact live method set after pairing rather than treating
a scope name as permission to call every method in that namespace.
