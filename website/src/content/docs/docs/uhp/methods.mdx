...
`pane.release_session` gives that binding back when the agent leaves the
session. It is fenced by identity: the pane, agent and session id must all still
name the current binding, so a release delayed behind a newer report can never
clear the newer session. A mismatch is not an error — the result reports
`released: false` and nothing changes. Releasing also drops usage this pane
reported for that exact session.

```json
{"id":"release","method":"pane.release_session","params":{"pane":"9","agent":"opencode","session_id":"ses_123"}}
{"id":"release","result":{"type":"ok","released":true}}
```
