...
- **Hook**, via `luvus integration install <agent>`, makes the agent report its
  ...
  cannot claim the pane. Leaving that session hands the exact binding back
  through `pane.release_session`, which only clears the identity the plugin
  itself reported. A failed report is retried with bounded backoff, and a newer
  report always wins over an older retry. The V2 plugin reports session
  identity, not usage.
