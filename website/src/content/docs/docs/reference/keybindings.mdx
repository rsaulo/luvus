...
In FILES, `o` or `Ctrl+O` opens the selected file or folder in the desktop's
application on the client machine. The path is passed as a separate argument;
application or shortcut-like entries (`.app`, `.desktop`, `.lnk`, `.command`)
run as a double-click would, so only open entries you trust.
