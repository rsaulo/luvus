...
pub(crate) struct GraphicsQueue {
    ...
    /// Where the transfer in progress begins in `commands` and `placements`.
    /// Abandoning it takes back what it queued and nothing that came before.
    transfer_start: Option<(usize, usize)>,
...
impl GraphicsQueue {
    ...
    pub(crate) fn push(
        ...
        // Only what that transfer queued is taken back: its opening chunk and
...
/// and a server it cannot vouch for could name anything. An embedded terminator
/// would end the sequence early and leave the remainder to be read as ordinary
/// terminal commands, so a command carrying one — or any other control byte —
/// is refused instead of written to the terminal.
pub(crate) fn is_wellformed_command(command: &[u8]) -> bool {
    ...
    // Keys, values, and base64 payload are all printable ASCII.
    !body.is_empty() && body.iter().all(|byte| (0x20..0x7f).contains(byte))
}
...
mod tests {
    ...
    /// A complete image, a transfer abandoned mid-chunk, then the command that
    /// abandoned it. Only the abandoned run is taken back: an image that was
    /// already whole is still owed to the terminal, and so is its replacement.
    #[test]
    fn an_abandoned_transfer_keeps_the_commands_queued_around_it() {
        let mut queue = GraphicsQueue::default();
        queue.push(b"a=t,i=1;AAAA", (0, 0), Some(CELL));
        queue.push(b"a=t,i=2,m=1;BBBB", (0, 0), Some(CELL));
        queue.push(b"m=1;BBBB", (0, 0), Some(CELL));
        queue.push(b"a=t,i=3;CCCC", (0, 0), Some(CELL));
        assert_eq!(
            queue.drain(),
            vec![wrapped("a=t,i=1;AAAA"), wrapped("a=t,i=3;CCCC")],
            "the image before the abandoned transfer survives it"
        );
        assert_eq!(
            queue.retained(),
            vec![wrapped("a=t,i=1;AAAA"), wrapped("a=t,i=3;CCCC")],
            "a late attach learns both whole images and neither partial one"
        );
    }

    /// The placements a resize appends while a transfer is in flight outlive
    /// that transfer being abandoned: they belong to images already held.
    #[test]
    fn an_abandoned_transfer_keeps_placements_queued_after_it() {
        let mut queue = GraphicsQueue::default();
        queue.push(b"a=t,i=1;AAAA", (0, 0), Some(CELL));
        queue.push(b"a=t,i=2,m=1;BBBB", (0, 0), Some(CELL));
        queue.queue(b"a=p,U=1,i=1,c=2,r=2", Retain::Place(1));
        ...
        assert_eq!(
            queue.drain(),
            vec![
                wrapped("a=t,i=1;AAAA"),
                wrapped("a=p,U=1,i=1,c=2,r=2"),
                wrapped("a=t,i=3;CCCC")
            ]
        );
    }
    ...
    /// What a display accepts from its server. An image is one whole APC and
    /// nothing else, so anything that could end the sequence early is refused.
    #[test]
    fn only_one_whole_kitty_sequence_is_written_to_a_terminal() {
        assert!(is_wellformed_command(&wrapped("a=T,f=100,i=1;AAAA")));
        assert!(is_wellformed_command(&wrapped("a=p,U=1,i=1,c=2,r=2")));

        assert!(!is_wellformed_command(b""), "nothing is not a command");
        assert!(
            !is_wellformed_command(&wrapped("")),
            "an empty payload names no image"
        );
        assert!(
            !is_wellformed_command(b"\x1b_Ga=T;AAAA"),
            "an unterminated sequence would swallow what follows it"
        );
        assert!(
            !is_wellformed_command(b"a=T;AAAA\x1b\\"),
            "a body without the introducer is not a graphics command"
        );
        assert!(
            !is_wellformed_command(b"\x1b_Ga=T;AAAA\x1b\\\x1b]0;pwned\x07"),
            "a second sequence must not ride along behind the first"
        );
        assert!(
            !is_wellformed_command(b"\x1b_Ga=T;AA\x1b\\rm -rf\x1b\\"),
            "an embedded terminator would end the image early"
        );
        assert!(
            !is_wellformed_command(b"\x1b_Ga=T;AA\x07BB\x1b\\"),
            "a stray control byte is not payload"
        );
        assert!(
            !is_wellformed_command("\x1b_Ga=T;é\x1b\\".as_bytes()),
            "payload is printable ASCII, never arbitrary bytes"
        );
    }
